...
for k in range(0,7):
    ...
    train_config = {'minibatch_size':64, 'actor_lr':0.0001, 'tau':0.001, 
                    'critic_lr':0.001, 'gamma':0.999, 'buffer_size':250000, 
                    'random_seed':int(time.perf_counter()*1000%1000), 'noise_sigma':noise_sigma, 'sigma2':SIGMA2}

    IS_TRAIN = False

    res_path = 'test_M_ddpg_sigma0_02_lane2_rate_'+ str(rate)+'/'
    step_result_path = res_path + 'step_result/'
    model_fold = 'model_M_ddpg_sigma0_02_rate3_lane2/'
    model_path = 'model_M_ddpg_sigma0_02_rate3_lane2/my_train_model_rate_3_1-2000'
    ...
    user_config = [{'id':'1', 'model':'AR', 'num_r':NUM_R, 'rate':rate-0.01, 'action_bound':1, 
                    'data_buf_size':100, 't_factor':t_factor, 'penalty':1000,'lane':2.0},
                   {'id':'2', 'model':'AR', 'num_r':NUM_R, 'rate':3, 'action_bound':1, 
                    'data_buf_size':100, 't_factor':t_factor, 'penalty':1000,'lane':1.0},
                   {'id':'3', 'model':'AR', 'num_r':NUM_R, 'rate':3, 'action_bound':2, 
                    'data_buf_size':100, 't_factor':t_factor, 'penalty':1000,'lane':2.0},
                   {'id':'4', 'model':'AR', 'num_r':NUM_R, 'rate':3, 'action_bound':2, 
