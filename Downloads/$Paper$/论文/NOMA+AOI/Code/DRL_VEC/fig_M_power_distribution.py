...
[step_r, step_p, step_b, step_lp, step_op]= read_log('test_M_ddpg_sigma0_02_lane2_rate_3.0/step_result/')

fig = plt.figure(figsize=(6, 4.5))
plt.plot(range(step_lp.shape[0]), step_lp, color='#ff7f0e', label='local', lw=1 )
plt.plot(range(step_op.shape[0]), step_op, color='#1f77b4', label='offload', lw=1 )
plt.grid(linestyle=':')
plt.legend()
plt.ylabel('Power')
plt.xlabel('Slot Index')
plt.show()
# fig.savefig('test_S_ddpg_sigma0_02_rate3_lane2/step_result/', format='eps', dpi=1000)
